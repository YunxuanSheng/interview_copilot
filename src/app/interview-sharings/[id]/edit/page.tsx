...
import { Building2, ArrowLeft, X, Plus } from "lucide-react"
...
export default function EditInterviewSharingPage({ params }: { params: { id: string } }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        router.push(`/interview-sharings/${params.id}`)
      } else {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      <div className="mb-6">
        <Link href={`/interview-sharings/${params.id}`} className="inline-flex items-center text-gray-600 hover:text-gray-900 mb-4">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex justify-end space-x-4">
          <Link href={`/interview-sharings/${params.id}`}>
