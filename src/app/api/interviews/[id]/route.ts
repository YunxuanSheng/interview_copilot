      ...
      const mockRecords = [
        {
          ...
          scheduleId: "3",
          audioFilePath: null,
          transcript: "面试官：你好，请先自我介绍一下。\n\n候选人：你好，我是张三，有3年Java开发经验，主要使用Spring框架开发过多个项目。我毕业于计算机科学专业，在校期间就接触了Java开发，毕业后一直专注于后端技术栈的学习和实践。\n\n面试官：能说说你对Spring的理解吗？\n\n候选人：Spring是一个轻量级的Java框架，提供了依赖注入和面向切面编程等功能。我在项目中主要使用Spring Boot来快速搭建微服务应用，通过@Autowired注解实现依赖注入，使用@Transactional注解管理事务。Spring的核心特性包括IoC容器、AOP面向切面编程、声明式事务管理等。\n\n面试官：你在项目中遇到过哪些性能问题，是如何解决的？\n\n候选人：在之前的电商项目中，我们遇到了高并发下的数据库查询性能问题。我通过添加数据库索引、使用Redis缓存、优化SQL查询语句等方式来解决。具体来说，我为经常查询的字段添加了复合索引，将热点数据缓存到Redis中，并使用了连接池来管理数据库连接。还使用了分页查询和异步处理来提升用户体验。\n\n面试官：你对微服务架构有什么了解？\n\n候选人：微服务架构是将单体应用拆分为多个独立的服务，每个服务负责特定的业务功能。我在项目中使用Spring Cloud来实现微服务，包括服务注册与发现、配置中心、网关等功能。微服务的优势是提高了系统的可扩展性和可维护性，但也带来了分布式事务、服务间通信等挑战。\n\n面试官：如何保证微服务的数据一致性？\n\n候选人：我们主要使用分布式事务的最终一致性方案。对于强一致性要求的场景，使用Seata框架实现分布式事务。对于最终一致性场景，使用消息队列和补偿机制，比如通过RocketMQ发送事务消息，确保数据最终一致。\n\n面试官：你了解哪些数据库优化策略？\n\n候选人：数据库优化可以从多个方面入手。首先是索引优化，合理设计单列索引和复合索引。其次是SQL优化，避免全表扫描，使用合适的JOIN方式。还有分库分表，根据业务特点进行水平或垂直拆分。另外还有读写分离、缓存策略等。\n\n面试官：你如何保证代码质量？\n\n候选人：首先建立代码规范，使用CheckStyle统一代码风格。其次编写单元测试，使用JUnit和Mockito测试业务逻辑。还有代码审查，通过Pull Request进行同行评审。最后是持续集成，每次提交都自动运行测试和构建，确保代码质量。\n\n面试官：你平时如何学习新技术？\n\n候选人：我主要通过官方文档、技术博客、开源项目来学习新技术。会关注一些技术社区，比如掘金、思否等。也会通过实际项目来实践新技术，遇到问题会查阅资料或向同事请教。还会参加一些技术会议和线上分享，了解行业动态。\n\n面试官：你对后端发展趋势有什么看法？\n\n候选人：我认为后端正在向云原生方向发展，容器化和Kubernetes让应用部署更灵活。微服务架构也越来越成熟，可以更好地支持大型应用。还有Serverless、边缘计算等新技术，让后端服务更接近用户。另外，AI和机器学习也在改变后端开发模式。\n\n面试官：你有什么问题要问我们吗？\n\n候选人：我想了解一下公司的技术栈和团队规模，以及这个岗位的具体职责。还有公司对新技术的接受程度，是否有技术分享和学习的氛围。\n\n面试官：我们主要使用Spring Cloud + MySQL的技术栈，团队有30人左右，这个岗位主要负责后端开发。我们鼓励技术创新，每周都有技术分享会。\n\n候选人：听起来很不错，我很期待能加入这样的团队。\n\n面试官：好的，今天的面试就到这里，我们会在一周内给你回复。\n\n候选人：谢谢，期待您的回复。",
